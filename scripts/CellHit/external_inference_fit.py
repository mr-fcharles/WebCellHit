...
def external_inference(drugID,
    ...
    with open(Path(f'/home/fcarli/CellHit/results/{dataset}/search_and_inference/moa_primed/models')/f'{int(drugID)}.pkl', 'rb') as f:
    ...
    #save the model
    save_path = Path(f'/home/fcarli/WebCellHit/results/CellHit/inference_models/{dataset}/')

    #check if the directory exists, if not create it
    if not (save_path/f'{int(drugID)}').exists():
        (save_path/f'{int(drugID)}').mkdir(parents=True)

    results['model'].save_model(save_path/f'{int(drugID)}/')


    #results['model'].save(Path(f'/home/fcarli/WebCellHit/results/CellHit/inference_models/{dataset}')/f'{int(drugID)}.pkl')
